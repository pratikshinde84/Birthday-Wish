...
function App() {
  ...
  return <>
    ...
    <header className="site-header"><a href="#top" className="brand" aria-label="Back to top"><span className="brand-mark">✦</span> a little note for you</a><nav className="main-nav" aria-label="Main navigation"><a href="#letter">The letter</a><a href="#memories">Our memories</a></nav><button className={`music-button ${isMusicOn ? 'active' : ''}`} type="button" aria-label="Toggle ambient music" onClick={() => { const next = !isMusicOn; setIsMusicOn(next); showToast(next ? 'Imagine your favorite song playing right now.' : 'A quiet little birthday moment.'); }}><span className="music-bars"><i /><i /><i /></span><span className="music-label">sound {isMusicOn ? 'on' : 'off'}</span></button></header>
    <main id="top">
